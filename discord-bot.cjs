...
// 3. Command Definitions (Selected list)
const commands = [
...
// 4. Register Commands & Clean Up Old Server Commands on Startup
client.once('clientReady', async () => {
  console.log(`Bot logged in as ${client.user.tag}`);
  const rest = new REST({ version: '10' }).setToken(process.env.DISCORD_TOKEN);

  try {
    // Force delete old guild-level commands from all joined servers
    for (const guild of client.guilds.cache.values()) {
      await rest.put(Routes.applicationGuildCommands(client.user.id, guild.id), { body: [] });
    }

    // Overwrite global slash commands
...
// 6. Command, Button, and Modal Handlers
  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (commandName === 'setup-verify') {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription('To access channels and participate in this server, complete verification below.')
        .setColor(0x2B2D31);
    ...
    if (commandName === 'setup-redeem') {
      const embed = new EmbedBuilder()
        ...
        .setDescription('Redeem your unique buyer key below to claim your Discord role.')
        .setColor(0x5865F2);
